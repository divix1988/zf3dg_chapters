...
class ComicsTable extends AbstractTable
{
    ...
    public function getById($id)
    {
         ...
         if (!$row) {
            throw new \Exception('comics not found with id: '.$id);
         }
         return $row;
    }
    ...
    {
        if (empty($data)) {
            throw new \Exception('no params has been provided for patch action');
        }
        $passedData = [];
    ...
    {
        if (empty($id)) {
            throw new \Exception('not passed comics id for delete action');
        }
        parent::deleteRow($id);
    }
}